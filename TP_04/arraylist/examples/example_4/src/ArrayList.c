...
/** \brief  read the size of arrayList
 * \param pList ArrayList* Pointer to arrayList
...
ArrayList* al_clone(ArrayList* this)
{
    ...
    if(this !=NULL)
    {
        ...
        for(j=0;j<i;j++)
        {
            returnAux->add(returnAux,this->get(this,j));
        }
    }
...
int al_containsAll(ArrayList* this,ArrayList* this2)
{
    ...
    if(this != NULL && this2 != NULL)
    {
        ...
        for(j=0;j<i;j++)
        {
            returnAux  = this->contains(this,*(this2->pElements+j));
            if(returnAux == 0)
            {
                break;
            }
...
/** \brief Decrement the number of elements.
 * \param pList ArrayList* Pointer to arrayList
 * \return int Return (-1) if Error [pList is NULL pointer or if can't allocate memory]
 *                  - (0) if ok
 */
int resizeDown(ArrayList* this)
{
    ...
    if(this!= NULL)
    {
        if(this->reservedSize-(this->size+AL_INCREMENT)>(this->size+AL_INCREMENT))
        {
            aux = (void**)realloc(this->pElements,sizeof(void*)*(this->size+AL_INCREMENT));
            if(aux != NULL)
            {
